...
import { flagDueDateModel } from './flagDueDateModel'
...
const moveCardToDifferentBoard = async (cardId, currentBoardId, currentColumnId, newBoardId, newColumnId, newPosition) => {
  try {
    ...
    const updatedMemberIds = [
      ...new Set([
        ...newBoard.ownerIds.filter(ownerId => card.memberIds.map(id => id.toString()).includes(ownerId.toString())),
    ...
    await GET_DB().collection(COLUMN_COLLECTION_NAME).findOneAndUpdate(
      ...
      { $push: { cardOrderIds: { $each: [card._id], $position: newPosition } } }
    )
...
const copyCard = async (cardId, currentBoardId, currentColumnId, newBoardId, newColumnId, newPosition, title, keepingItems) => {
  try {
    ...
    if (keepingItems.includes('memberIds')) {
      ...
      newCard.memberIds = [
        ...new Set([
          ...newBoard.ownerIds.filter(ownerId => originalCard.memberIds.map(id => id.toString()).includes(ownerId.toString())),
    ...
    await GET_DB().collection(COLUMN_COLLECTION_NAME).findOneAndUpdate(
      { _id: new ObjectId(newColumnId) },
      { $push: { cardOrderIds: { $each: [result.insertedId], $position: newPosition } } }
    )
...
const updateDueDate = async (cardId, dueDateData) => {
  try {
...
export const cardModel = {
  ...
  copyCard, deleteCardById, deleteCardsByBoardId, updateDueDate
}