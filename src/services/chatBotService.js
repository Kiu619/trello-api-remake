...
13. GENERAL_QUERY (câu hỏi chung): Câu hỏi hoặc trò chuyện chung không liên quan đến các hành động trên

Nếu thuộc các loại từ 1-11, hãy trích xuất thông tin cần thiết tương ứng:
...
- CLOSE_COLUMN: columnTitles (mảng tên các cột cần đóng), isAllColumns (boolean)
- OPEN_COLUMN: columnTitles (mảng tên các cột cần mở), isAllColumns (boolean)

Trả về JSON có cấu trúc:
{
  'intent': 'MOVE_CARD|COPY_CARD|DELETE_CARD|CREATE_CARD|CARD_DETAILS|UPDATE_CARD|BOARD_SUMMARY|CREATE_COLUMN|COPY_COLUMN|MOVE_ALL_CARDS|CLOSE_COLUMN|GENERAL_QUERY',
  'data': {
...
- Với CLOSE_COLUMN, nếu người dùng muốn đóng tất cả các cột, hãy đặt isAllColumns = true, nếu đóng một số cột cụ thể, hãy đặt columnTitles là mảng chứa tên các cột cần đóng
- Với OPEN_COLUMN, nếu người dùng muốn mở tất cả các cột, hãy đặt isAllColumns = true, nếu mở một số cột cụ thể, hãy đặt columnTitles là mảng chứa tên các cột cần mở`
  ...
  async chat(message, userId, boardId) {
    try {
      ...
      case 'CLOSE_COLUMN': {
        ...
        const response = 'Vui lòng cho biết tên cột bạn muốn đóng. Ví dụ: "Đóng cột \'Tên cột\'" hoặc "Đóng tất cả cột".'
        await this.saveChat(userId, boardId, message, response)
        return response
      }
      ...
      case 'OPEN_COLUMN': {
        ...
        // Nếu không đủ thông tin, yêu cầu người dùng cung cấp thêm
        const response = 'Vui lòng cho biết tên cột bạn muốn mở. Ví dụ: "Mở cột \'Tên cột\'" hoặc "Mở tất cả cột".'
        await this.saveChat(userId, boardId, message, response)
        return response
      }
